...
public class OngroundMask extends RichParser<Mask> {
    ...
    protected Stream<String> getSuggestions(String paramString, int paramInt) {
        ...
        return (paramInt == 1) ? this.worldEdit.getMaskFactory().getSuggestions(paramString).stream() : ((paramInt == 0) ? this.worldEdit.getMaskFactory().getSuggestions(paramString).stream() : null);
    }
